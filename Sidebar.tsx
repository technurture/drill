      ...
      <aside
        ...
        >
          ...
          <div className="flex flex-col flex-grow overflow-y-auto">
            ...
            <nav className={`px-4 ${collapsed ? 'space-y-3' : 'space-y-2'}`}>
              {navItems.map((item) => {
                ...
                const isSalesItem = item.path === "/dashboard/sales";
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                  ...
                  >
                    ...
                    {isSalesItem && (
                      <Badge className={`absolute ${collapsed ? '-top-1 -right-1' : '-top-1 -right-1'} bg-red-500 hover:bg-red-500 text-white text-xs h-5 w-5 p-0 flex items-center justify-center rounded-full`}>
                        0
                      </Badge>
                    )}
                  </Link>
          ...
          <div className={`p-4 border-t border-gray-200 dark:border-gray-800 space-y-2`}>
            <Link
              to="https://preview--shebalance-help-dock.lovable.app/getting-started"
              target="_blank"
