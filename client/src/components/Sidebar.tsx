...
const Sidebar = ({ onClose, isSideBar }) => {
  ...
  return (
    <div className={`${isSideBar && "bg-transparent w-screen"}`} onClick={onClose}>
      <aside
        className={`bg-white dark:bg-[#18191A] h-screen flex-shrink-0 flex flex-col justify-between transition-all duration-300 relative lg:static z-50 border-r border-gray-200 dark:border-gray-800 ${collapsed ? "w-[80px]" : "w-[280px]"
          }`}
      >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  to={item.path}
                  onClick={onClose}
                  className={`group relative flex items-center ${collapsed ? 'justify-center' : 'space-x-3'} p-3 rounded-xl transition-all duration-200 hover:bg-gray-50 dark:hover:bg-gray-800/50 ${isActive
                      ? "bg-gray-100 dark:bg-gray-800 shadow-sm"
                      : ""
                    }`}
                  title={collapsed ? item.label : undefined}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${isActive ? item.activeColor : item.color
                    }`}>
                    <Icon size={16} />
                  </div>
                  {!collapsed && (
